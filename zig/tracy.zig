...
pub const enable_callstack = enable and (if (@hasDecl(options_override, "enable_callstack")) options_override.enable_callstack else false);
const callstack_depth = if (@hasDecl(options_override, "callstack_depth")) options_override.callstack_depth else 10;

const options_override = if (@hasDecl(@import("root"), "tracy_options"))
